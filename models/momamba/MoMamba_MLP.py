...
from models.momamba.UNetBlock import PositionalEncoding,RotaryPositionalEmbeddings
from models.momamba.MoMGate import *

class MLP(nn.Module):
    def __init__(self, input_size, ratio=4):
        super(MLP, self).__init__()

        output_size=input_size
        hidden_size=int(input_size/ratio)
...
class MoMBlock(nn.Module):
    def __init__(self, dim, num_experts,top_k,emb_type="PE",head=2,use_aux_loss=False):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        ...
        aux_loss=0
        if isinstance(self.mom,(MoMEncoder)):
            x_mom, aux_loss =self.mom(x_norm)
        else:
            x_mom = self.mom(x_norm)

        x_mom =  x+self.gamma* x_mom
        x = x_mom.reshape(B, H, W, D, C).permute(0, 4, 1, 2, 3)  # (B, C, H, W, D)
        return x, aux_loss
...
class MoMEncoder(nn.Module):
    ...
    def forward(self, x):
        if self.emb_type =="PE" or self.emb_type =="RPE":
            x=self.pos_emb(x)
...
class StageEncoder(nn.Module):
    def __init__(self, in_channels,out_channels,num_experts,top_k,
        ...
        for j in range(depth):
            stage_blocks.append(MoMCodec(out_channels,num_experts[j],top_k))  
...
class StageDecoder(nn.Module):
    def     __init__(
        ...
        if isconv:
            ...
            decoder_block = []
            for j in range(depth):
                decoder_block.append(MoMCodec(out_channels,num_experts[j],top_k))
...
class MoMamba_MLP(nn.Module):
    ...
    def __init__(
        ...
        self.conv10 = StageEncoder(in_channels, dims[1], num_experts[0], top_k)
        self.conv20 = StageEncoder(dims[1], dims[2], num_experts[1], top_k)
        self.conv30 = StageEncoder(dims[2], dims[3], num_experts[2], top_k)
        self.conv40 = StageEncoder(dims[3], dims[4], num_experts[3], top_k)
        ...
        self.up_concat31 = StageDecoder(dims[4], dims[3],2,False, num_experts[2], top_k)

        self.up_concat02 = StageDecoder(dims[1], dims[0],3)
        self.up_concat12 = StageDecoder(dims[2], dims[1],3)
        self.up_concat22 = StageDecoder(dims[3], dims[2],3,False, num_experts[1], top_k)

        self.up_concat03 = StageDecoder(dims[1], dims[0],4)
        self.up_concat13 = StageDecoder(dims[2], dims[1],4,False, num_experts[0], top_k)
